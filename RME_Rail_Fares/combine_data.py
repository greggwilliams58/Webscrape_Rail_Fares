...
def tidyupfiles(dailydatapath, appendeddatapath):
    ...
    Returns:
    None                But manipulates files through appending data and deleting files.  Exports appended data for all, fixed, relative search types 
    """
    dailydataname = 'RME_data_collected_for*'
    appendeddataname = 'all_appended_data'
    fixeddataname = 'fixed_appended_data'
    relativedataname = 'relative_appended_data'

    fileextension = '.csv'

    #get the daily dataset for all search types
    dailydata = get_daily_data(dailydatapath,dailydataname,fileextension)
    
    #get the three types of append data
    allappendeddata = get_appended_data(appendeddatapath,appendeddataname,fileextension)
    fixedappendeddata = get_appended_data(appendeddatapath, fixeddataname, fileextension)
    relativeappendeddata = get_appended_data(appendeddatapath, relativedataname, fileextension)

    # append all of the daily data to each of the appended data files and then filter by search type
    alldata = combine_daily_and_appended_data(dailydata, allappendeddata)
    fixeddata = combine_daily_and_appended_data(dailydata,fixedappendeddata,'Fixed')
    relativedata = combine_daily_and_appended_data(dailydata, relativeappendeddata, 'Relative')
    ...
    fixeddata.to_csv(appendeddatapath + fixeddataname + "_for_" +todaysdate + fileextension)
    relativedata.to_csv(appendeddatapath + relativedataname + "_for_" +todaysdate + fileextension)
...
def get_appended_data(filepath, filename, fileextension):
    ...
    """
    list_of_files = glob(filepath + filename + '*.csv') # * means all if need specific format then *.csv
...
def combine_daily_and_appended_data(dailydata, appendeddata, appendtype = None):
    """
    This appends the daily data to the appropriate appended data file, renaming the new index as 'general index'
    ...
    if appendtype == 'Fixed':
        fixed_data = all_data[all_data['Search_Type'].map(lambda x : x.startswith('Fixed'))]
        fixed_data.reset_index(drop=True,inplace=True)
        fixed_data.rename_axis('general_index',axis='index',inplace=True)
        return fixed_data
    elif appendtype == 'Relative':
        relative_data = all_data[all_data['Search_Type'].map(lambda x : x.startswith('Relative'))]
        relative_data.reset_index(drop=True,inplace=True)
        relative_data.rename_axis('general_index',axis='index',inplace=True)
